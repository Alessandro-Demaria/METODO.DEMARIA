...
Modulo: verify_markov.py (Analizzatore Stocastico Vettorizzato v2.02)
...
Descrizione:
  ...
  Calcola la matrice delle probabilita di transizione di primo ordine tra gli stati
...
class MarkovVerifier:
    """
    Analizzatore stocastico vettorizzato per le Catene di Markov del Metodo Demaria.
    ...
    def compute_stationary_distribution_numpy(cls, transition_matrix: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # Trova l'autovalore piu vicino a 1.0
            idx = np.argmin(np.abs(eigenvalues - 1.0))
            stationary = np.real(eigenvectors[:, idx])
            stationary = stationary / np.sum(stationary)
            # Garantisce probabilita non-negative
    ...
    def analyze_vector_array(self, state_array: np.ndarray, line_boundaries: Optional[np.ndarray] = None) -> Dict[str, Any]:
        ...
        :return: Dizionario contenente matrice \(4 \times 4\) e distribuzione stazionaria.
        """
        ...
        # Maschera intra-linea per isolare i confini di riga
    ...
    def analyze_corpus(self, raw_text: str) -> Dict[str, Any]:
        """
        Analizza le proprieta di Markov sull'intero testo fornito con filtro di riga.
        """
    ...
    def analyze_file(self, file_path: str) -> Dict[str, Any]:
        """
        Metodo d'istanza per eseguire l'analisi markoviana da file sanificato.
        """
...
if __name__ == '__main__':
    print("=" * 75)
    print("METODO DEMARIA — VERIFICA INTEGRITÀ MARKOV ANALYSIS (v2.02)")
    print("=" * 75)
    ...
    print("\n[✓] ESITO VERIFICA: verify_markov.py (v2.02) VALIDO E CONFORME AL 100%.")
    print("=" * 75)